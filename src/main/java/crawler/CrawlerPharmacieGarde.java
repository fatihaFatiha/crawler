...
public class CrawlerPharmacieGarde {
	...
	public void crawlPharmacieDeGarde() {
		...
		try {
			...
			Document doc = Jsoup.connect(url).timeout(0).userAgent("Chrome")
					.get();
			Elements names = doc
					.select(".pharmacies_elements_cls #resultats_h3_span .h2_rs_st_pnl_home .moodalbox");
			Elements addresses = doc
					.select(".pharmacies_elements_cls>div span[itemprop=\"streetAddress\"]");
			Elements tels = doc
					.select(".pharmacies_elements_cls div:nth-child(2) .tel_services");
			...
			for (int i = 0; i < liste1.size(); i = i + 1) {
				...
				for (String nom : listeNoms) {
					...
					String nom1 = liste1.get(i).replaceAll(" ", "")
							.toLowerCase();
	...
	public String getCrawlUrl() {
		return "http://www.telecontact.ma/services/pharmacies-de-garde/agadir-Maroc";
